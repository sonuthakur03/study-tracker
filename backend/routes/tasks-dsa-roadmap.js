...
dsaRouter.get('/', auth, async (req, res) => {
  try {
    const questions = await DSAQuestion.find().sort({ dayNumber: 1, createdAt: 1 });
...
dsaRouter.post('/', adminAuth, async (req, res) => {
  try {
    const q = await DSAQuestion.create(req.body);
...
dsaRouter.put('/:id', adminAuth, async (req, res) => {
  try {
    const q = await DSAQuestion.findByIdAndUpdate(req.params.id, req.body, { new: true });
    res.json(q);
  } catch (err) { res.status(400).json({ message: err.message }); }
});
...
dsaRouter.post('/:id/toggle', auth, async (req, res) => {
  try {
    ...
    await q.save();
    res.json({ completed: idx === -1, message: idx === -1 ? 'Marked complete' : 'Marked incomplete' });
  } catch (err) { res.status(500).json({ message: err.message }); }
});
...
roadmapRouter.put('/:id', adminAuth, async (req, res) => {
  try {
    const topic = await RoadmapTopic.findByIdAndUpdate(req.params.id, req.body, { new: true });
    res.json(topic);
  } catch (err) { res.status(400).json({ message: err.message }); }
});
...
roadmapRouter.delete('/:id', adminAuth, async (req, res) => {
  try {
    await RoadmapTopic.findByIdAndDelete(req.params.id);
...
roadmapRouter.post('/:id/toggle', auth, async (req, res) => {
  try {
    ...
    // Update user phase
    const completedInPath = await RoadmapTopic.countDocuments({
      path: topic.path,
      completedBy: uid,
    });
    const totalInPhase = await RoadmapTopic.countDocuments({ path: topic.path, phase: topic.phase });
    const completedInPhase = await RoadmapTopic.countDocuments({ path: topic.path, phase: topic.phase, completedBy: uid });
